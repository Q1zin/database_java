...
public class FXMLController_old {
    ...
    void initialize() {
        ...
        tabView.getTabPane().getSelectionModel().selectedItemProperty().addListener((obs, oldTab, newTab) -> {
            if (newTab == tabView) {
                System.out.println("Вкладка tabView выбрана!");
                loadTabView();
            }
        });

        btnDoSql.setOnMouseClicked(event -> {
            String sql = textareaSql.getText();
            String nameDB = getSelectDb();

            if (sql.isEmpty() || nameDB.isEmpty()) { return; }

            boolean isSelect = sql.split(" ")[0].equals("SELECT");

            try {
                if (isSelect) {
                    loadTabView(sql);
                    showNotify("Запрос успешно выполнен");
                    return;
                }
                pdo.executeSQL(sql, nameDB);

                showNotify("Запрос успешно выполнен");
                System.out.println("Проверка результата" + pdo.getResiltSql());
